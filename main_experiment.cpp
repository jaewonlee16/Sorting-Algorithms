...
int main(int argc, char* argv[]) {
    ...
    cout << "{\"n\": " << n << ", \"seed\": " << seed << ", \"algorithm\": \"";    

    begin = system_clock::now();

    int* sorted;

    int sort_algorithm = stoi(argv[1]);
    // Select sorting algorithm
    if (sort_algorithm == 0){
        cout << "insertion sort";
        sorted = insertion_sort(arr, n);
    }
    else if (sort_algorithm == 1){
        cout << "merge sort";
        sorted = merge_sort(arr, n);
    }
    else if (sort_algorithm == 2){
        cout << "quick sort rand";
        sorted = quick_sort_rand_piv(arr, n);
    }
    else if (sort_algorithm == 3){
        cout << "quick sort last";
        sorted = quick_sort_last_piv(arr, n);
    }
    else if (sort_algorithm == 4){
        cout << "heap sort";
        sorted = heap_sort(arr, n);
    }
    else if (sort_algorithm == 5){
        cout << "radix sort";
        sorted = radix_sort(arr, n);
    }
    else if (sort_algorithm == 6){
        cout << "stooge sort";
        sorted = stooge_sort(arr, n);
    }

    duration = duration_cast<milliseconds>(system_clock::now() - begin).count() / 1000.0;
    cout << "\", \"sort_time\": " << duration;

    is_sorted = check_sorted(sorted, n);
    cout << ", \"Sorted\": " << check_sorted(sorted, n) << "}" << endl;
    delete[] sorted;
